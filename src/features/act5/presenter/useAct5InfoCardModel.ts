...
import type { Act5InfoCardMode, Act5InfoCardModel } from "~/types/act5InfoCard";
...
export const getAct5FeedbackTextKey = (
  ...
  if (!feedbackCode) return `story.acts.act5.${season}Feedback.TRY_AGAIN`;

  return `story.acts.act5.${season}Feedback.${feedbackCode}`;
};
...
export const useAct5InfoCardModel = ({
  ...
  const feedback = computed<Act5InfoCardModel["feedback"]>(() => {
    ...
    if (store.attempt.retryPreviewFeedbackText) {
      return {
    ...
    if (store.feedback.visibleMeasureResult) {
  ...
  return {
    ...
    getFeedbackText: (target: { season: ClimateSeason }) =>
      t(getAct5FeedbackTextKey(target.season, recognition.feedbackCode.value)),
  };
};
