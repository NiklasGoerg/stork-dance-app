<template>
  <section
    class="act4-climate-chart"
    :class="[
      `act4-climate-chart--${chartMode}`,
      {
        'act4-climate-chart--inactive': isInactive,
        'act4-climate-chart--interaction-focus': isInteractionFocus,
        'act4-climate-chart--transition-focus': isTransitionFocus,
        'act4-climate-chart--feedback-burst': feedbackBurstActive,
        [`act4-climate-chart--feedback-${feedbackBurstLevel}`]:
          feedbackBurstActive,
        'act4-climate-chart--resetting': resetFeedbackActive,
        'act4-climate-chart--completed-summary': isCompletedSummary,
      },
    ]"
  ...
  >
    <div class="act4-climate-chart__header">
      <div class="act4-climate-chart__heading">
        ...
        <h2 id="act4-progress-chart-title">{{ titleLabel }}</h2>
      </div>

      <div class="act4-climate-chart__meta">
        <span class="act4-climate-chart__view">{{ viewLabel }}</span>
        <ul
          class="act4-climate-chart__legend"
          :aria-label="t('story.acts.act4.chart.legend')"
        >
          <li
    ...
    <div
      ...
      class="act4-climate-chart__plot"
      aria-hidden="true"
    >
      <svg
        ...
        role="img"
      >
        <g class="act4-climate-chart__grid">
          <line
        ...
        <rect
          ...
          class="act4-climate-chart__period-highlight"
        ...
        <line
          ...
          class="act4-climate-chart__baseline"
          :x1="plotLeft"
          :x2="plotRight"
          :y1="yScale(0)"
          :y2="yScale(0)"
        ...
        <g class="act4-climate-chart__axis">
          <line
            :x1="plotLeft"
            :x2="plotRight"
            :y1="plotBottom"
          ...
          <text
            class="act4-climate-chart__axis-label"
            :x="plotLeft + (plotRight - plotLeft) / 2"
            :y="chartHeight - 8"
            text-anchor="middle"
          >
            {{ t("story.acts.act4.chart.axis.period") }}
          </text>
          <text
            class="act4-climate-chart__axis-label"
          ...
          <g
            ...
            :key="tick.interval"
            :class="[
              'act4-climate-chart__x-tick',
              { 'act4-climate-chart__x-tick--future': tick.isFuture },
            ]"
          >
            <line
        ...
        <g class="act4-climate-chart__segments">
          <line
            v-for="segment in renderedSegments"
            :key="segment.renderKey"
            :class="[
              'act4-climate-chart__segment',
              `act4-climate-chart__segment--${segment.state}`,
              {
                'act4-climate-chart__segment--active': segment.active,
                'act4-climate-chart__segment--muted': segment.muted,
                'act4-climate-chart__segment--recent': segment.recent,
                'act4-climate-chart__segment--burst': segment.burst,
              },
            ]"
            :x1="segment.x1"
            :y1="segment.y1"
        ...
        <g class="act4-climate-chart__points">
          <circle
            v-for="point in renderedPoints"
            :key="point.renderKey"
            :class="[
              'act4-climate-chart__point',
              `act4-climate-chart__point--${point.state}`,
              {
                'act4-climate-chart__point--active': point.active,
                'act4-climate-chart__point--muted': point.muted,
                'act4-climate-chart__point--pulse': point.pulse,
                'act4-climate-chart__point--recent': point.recent,
              },
            ]"
            :cx="point.x"
            :cy="point.y"
        ...
        <g v-if="activeValueLabel" class="act4-climate-chart__value-label">
          <text
    ...
    <div v-else class="act4-climate-chart__empty">
      <p>{{ emptyLabel }}</p>
    </div>
  </section>
</template>
...
import type { Act4ClimateProgressChartProps } from "~/types/act4";
...
type ClimateMovementSeasonDataRow = ClimateSeasonDataRow & {
  season: ClimateSeason;
};

type PointGeometry = {
  id: string;
  row: ClimateMovementSeasonDataRow;
...
const inactiveFlowIds = new Set(["act4TutorialDebug"]);
...
const seasonSeries: SeasonConfig[] = [
  {
    ...
    color: "var(--act4-chart-winter)",
  },
  {
    season: "spring",
    label: t("seasonClock.seasons.spring"),
    color: "var(--act4-chart-spring)",
  },
  {
    season: "summer",
    label: t("seasonClock.seasons.summer"),
    color: "var(--act4-chart-summer)",
  },
  {
    season: "autumn",
    label: t("seasonClock.seasons.autumn"),
    color: "var(--act4-chart-autumn)",
  },
];

const isClimateMovementSeasonDataRow = (
  row: ClimateSeasonDataRow,
): row is ClimateMovementSeasonDataRow => isClimateMovementSeason(row.season);
...
const chartRows = computed<ClimateMovementSeasonDataRow[]>(() =>
  props.rows
    .filter(isClimateMovementSeasonDataRow)
    .sort(
      (a, b) =>
...
const rowByStepId = computed(() => {
  const rows = new Map<string, ClimateMovementSeasonDataRow>();
...
const activeRow = computed<ClimateMovementSeasonDataRow | null>(() =>
  activeStepId.value
...
const getSeasonConfig = (season: ClimateSeason): SeasonConfig => {
  const config = seasonSeries.find((item) => item.season === season);

  if (!config) {
    throw new Error(`Missing climate chart config for season "${season}".`);
  }

  return config;
};
const getPointGeometry = (row: ClimateMovementSeasonDataRow): PointGeometry => {
...
const toRenderPoint = (row: ClimateMovementSeasonDataRow): RenderPoint => {
...
const snapshotRows = computed<ClimateMovementSeasonDataRow[]>(() => {
  const visibleThrough =
    transitionPreviousIntervalOrder.value ?? baselineIntervalOrder.value;

  return chartRows.value.filter((row) => row.intervalOrder <= visibleThrough);
});
const completedRows = computed<ClimateMovementSeasonDataRow[]>(() =>
  chartRows.value.filter((row) => completedStepIdSet.value.has(getStepId(row))),
);
const visibleRows = computed<ClimateMovementSeasonDataRow[]>(() => {
...
const getSeasonRows = (
  rows: ClimateMovementSeasonDataRow[],
  season: ClimateSeason,
) =>
  rows
...
const createSegment = ({
  ...
  from: ClimateMovementSeasonDataRow;
  to: ClimateMovementSeasonDataRow;
  state: RenderSegmentState;
...
const eyebrowLabel = computed(() =>
  isInactive.value
    ? t("story.acts.act4.chart.eyebrow.inactive")
    : t("story.acts.act4.chart.eyebrow.active"),
);
const titleLabel = computed(() => {
  if (isCompletedSummary.value) {
    return t("story.acts.act4.chart.title.completed");
  }

  if (chartMode.value === "referenceAbsolute") {
    return t("story.acts.act4.chart.title.referenceAbsolute");
  }
  if (chartMode.value === "absoluteToDelta") {
    return t("story.acts.act4.chart.title.absoluteToDelta");
  }
  if (chartMode.value === "absoluteSnapshot") {
    return t("story.acts.act4.chart.title.absoluteSnapshot");
  }
  if (chartMode.value === "delta") {
    return t("story.acts.act4.chart.title.delta");
  }

  return t("story.acts.act4.chart.title.inactive");
});
const viewLabel = computed(() => {
  if (isCompletedSummary.value) return t("story.acts.act4.chart.view.summary");
  if (chartMode.value === "referenceAbsolute") {
    return t("story.acts.act4.chart.view.absolute");
  }
  if (chartMode.value === "absoluteSnapshot") {
    return t("story.acts.act4.chart.view.snapshot");
  }
  if (chartMode.value === "absoluteToDelta") {
    return t("story.acts.act4.chart.view.transition");
  }
  if (chartMode.value === "delta") return t("story.acts.act4.chart.view.delta");

  return t("story.acts.act4.chart.view.inactive");
});
const yAxisLabel = computed(() =>
  chartView.value === "absolute"
    ? t("story.acts.act4.chart.axis.temperature")
    : t("story.acts.act4.chart.axis.change"),
);
const emptyLabel = computed(() =>
  props.phase === "tutorial"
    ? t("story.acts.act4.chart.empty.tutorial")
    : t("story.acts.act4.chart.empty.story"),
);
...
.act4-climate-chart--interaction-focus {
  border-color: rgba(31, 49, 39, 0.16);
  box-shadow: 0 18px 44px rgba(31, 49, 39, 0.12);
}

.act4-climate-chart--transition-focus,
.act4-climate-chart--completed-summary {
...
.act4-climate-chart--feedback-burst {
  border-color: rgba(31, 49, 39, 0.22);
}

.act4-climate-chart--resetting {
  border-color: rgba(94, 108, 103, 0.2);
}

.act4-climate-chart__header {
  display: flex;
  align-items: end;
...
.act4-climate-chart__heading {
  min-width: 0;
}

.act4-climate-chart__eyebrow,
.act4-climate-chart h2,
.act4-climate-chart__empty p {
  margin: 0;
}
...
.act4-climate-chart__legend span {
  width: 17px;
  height: 3px;
  border-radius: 999px;
...
.act4-climate-chart__plot,
.act4-climate-chart__empty {
  min-width: 0;
  min-height: 0;
}
...
.act4-climate-chart__grid line {
  stroke: rgba(31, 49, 39, 0.055);
  stroke-width: 1;
}

.act4-climate-chart__period-highlight {
  fill: rgba(255, 255, 255, 0.58);
  stroke: rgba(31, 49, 39, 0.12);
  stroke-width: 1;
}

.act4-climate-chart__baseline {
  stroke: rgba(31, 49, 39, 0.36);
  stroke-dasharray: 5 6;
  stroke-width: 1.2;
}

.act4-climate-chart__axis line {
  stroke: rgba(31, 49, 39, 0.14);
  stroke-width: 1;
}

.act4-climate-chart__axis text {
...
.act4-climate-chart__axis .act4-climate-chart__axis-label {
  fill: rgba(31, 49, 39, 0.56);
  font-size: 11.5px;
  font-weight: 850;
}

.act4-climate-chart__x-tick--future text,
.act4-climate-chart__x-tick--future line {
  opacity: 0.34;
}
...
.act4-climate-chart__segment--preview {
  stroke: rgba(105, 116, 111, 0.42);
  stroke-dasharray: 6 7;
  stroke-width: 3;
}

.act4-climate-chart__segment--partial {
  stroke-width: 4.2;
}

.act4-climate-chart__segment--active {
  stroke-width: 5;
  opacity: 1;
}

.act4-climate-chart__segment--muted {
  opacity: 0.16;
  filter: saturate(0.25);
}

.act4-climate-chart__segment--recent {
  opacity: 1;
  stroke-width: 4.8;
}

.act4-climate-chart__segment--burst {
  animation: act4-chart-line-burst 760ms ease-out;
}

.act4-climate-chart--transition-focus .act4-climate-chart__segment {
  stroke-width: 4;
  opacity: 0.74;
}

.act4-climate-chart--transition-focus .act4-climate-chart__segment--recent {
  stroke-width: 5.2;
  opacity: 1;
}

.act4-climate-chart--completed-summary .act4-climate-chart__segment {
  stroke-width: 4.2;
  opacity: 0.84;
}
...
.act4-climate-chart__point--preview {
  fill-opacity: 0.18;
  stroke: rgba(93, 105, 100, 0.78);
}

.act4-climate-chart__point--partial {
  fill-opacity: 0.2;
  stroke-width: 3.4;
}

.act4-climate-chart__point--completed {
  fill-opacity: 0.96;
}

.act4-climate-chart__point--active {
  opacity: 1;
  filter: drop-shadow(0 0 8px rgba(31, 49, 39, 0.18));
  stroke-width: 3.6;
}

.act4-climate-chart__point--muted {
  opacity: 0.24;
  filter: saturate(0.25);
}

.act4-climate-chart__point--recent {
  opacity: 1;
  stroke-width: 3.2;
}

.act4-climate-chart__point--pulse {
  animation: act4-chart-point-pulse 720ms ease-out;
}

.act4-climate-chart--feedback-burst .act4-climate-chart__segment--active {
  filter: drop-shadow(0 0 7px rgba(31, 49, 39, 0.24));
  stroke-width: 6.1;
}

.act4-climate-chart--feedback-burst .act4-climate-chart__point--active {
  filter: drop-shadow(0 0 12px rgba(31, 49, 39, 0.3));
}

.act4-climate-chart--feedback-extra .act4-climate-chart__point--active {
  animation: act4-chart-point-glow 760ms ease-out;
}

.act4-climate-chart--resetting .act4-climate-chart__segment--preview,
.act4-climate-chart--resetting .act4-climate-chart__point--preview {
  transition-duration: 520ms;
}

.act4-climate-chart__value-label text {
...
@keyframes act4-chart-point-pulse {
  0% {
...
@keyframes act4-chart-line-burst {
  0% {
...
@keyframes act4-chart-point-glow {
  0% {
    filter: drop-shadow(0 0 0 rgba(31, 49, 39, 0));
  }
