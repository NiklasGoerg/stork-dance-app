...
describe("Act 5 story narration", () => {
  it("defines the fixed authored target cue inventory", () => {
    expect(act5StoryTargetCueIds).toHaveLength(24);
    expect(act5StoryTargetCueIds).toEqual([
      ...
      "act5.story.2020_2024.summer",
      "act5.story.2020_2024.autumn",
    ] satisfies Act5StoryTargetCueId[]);
  });

  it("defines period transition and completion cue inventories", () => {
    expect(act5StoryPeriodTransitionCueIds).toEqual([
    ...
    expect(act5StoryCompletionCueIds).toEqual([
  ...
  it("points every story narration cue to an English i18n string", () => {
    Object.values(act5StoryNarrationCatalog).forEach((cue) => {
  ...
  it("marks seasonal target cues as speak-only", () => {
    act5StoryTargetCueIds.forEach((cueId) => {
    ...
    expect(act5StoryNarrationCatalog["act5.story.intro.chart"].display).toBe(
      true,
    );
    expect(
      act5StoryNarrationCatalog["act5.story.reference.complete"].display,
    ).toBe(true);
  });

  it("keeps the post-reference scale cue as a short reminder", () => {
    const scaleText =
      en.story.acts.act5.narration.story.referenceComplete.scale;
  ...
  it("resolves comparison period transitions without adding a final transition", () => {
    expect(resolveAct5StoryPeriodTransitionCue("1995-1999")).toBeNull();
    expect(resolveAct5StoryPeriodTransitionCue("2000-2004")?.id).toBe(
      "act5.story.transition.2000_2004",
    );
    expect(resolveAct5StoryPeriodTransitionCue("2005-2009")?.id).toBe(
      "act5.story.transition.2005_2009",
    );
    expect(resolveAct5StoryPeriodTransitionCue("2010-2014")?.id).toBe(
      "act5.story.transition.2010_2014",
    );
    expect(resolveAct5StoryPeriodTransitionCue("2015-2019")?.id).toBe(
      "act5.story.transition.2015_2019",
    );
    expect(resolveAct5StoryPeriodTransitionCue("2020-2024")).toBeNull();
  });

  it("resolves every fixed story target to one authored cue", () => {
    const targets = buildAct5ClimateStorySequence(readDataset());
    const resolutions = targets.map(resolveAct5StoryNarrationCue);
  ...
  it("preserves the known semantic landmarks", () => {
    ...
    expect(resolveAct5StoryNarrationCue(winter2010!)?.cue.id).toBe(
      "act5.story.2010_2014.winter",
    );
    expect(resolveAct5StoryNarrationCue(winter2010!)?.params.deltaAbs).toBe(
      "point three",
    );
    expect(
    ...
    expect(resolveAct5StoryNarrationCue(winter2020!)?.cue.id).toBe(
      "act5.story.2020_2024.winter",
    );
  });
