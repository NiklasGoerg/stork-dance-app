import type {
  ...
  Act4FeedbackSelectionOptions,
  Act4SelectedFeedback,
} from "~/types/act4";
import type { ClimateSeason } from "~/types/climate";
import {
  buildAct4BeatFeedbackSignals,
  getAct4SignalMetadata,
} from "~/utils/act4/feedback/signals";
...
export const selectAct4FinalFeedback = <TCode extends string>({
  season,
  beatEvaluations,
  codeMetadata,
  criterionMetadata = {},
  fallbackCode,
}: Act4FeedbackSelectionOptions<TCode>): Act4SelectedFeedback<TCode> => {
  ...
  const signals = beatEvaluations.flatMap((evaluation) => {
    ...
    return buildAct4BeatFeedbackSignals({
      season,
