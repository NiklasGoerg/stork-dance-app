...
import {
  act4IntroCycleConfig,
  resolveAct4SeasonMovementConfig,
} from "~/story/act4IntroCycle";
import {
  buildAct4ClimateStorySequence,
  buildAct4DebugSeasonValueSequence,
  buildAct4DebugSingleSeasonSequence,
  buildAct4FullSequence,
  buildAct4TutorialDebugSequence,
} from "~/utils/act4/sequence";
import { getAct4EvaluationTransition } from "~/utils/act4/transitions";
import {
  resolveAct4SeasonCompletionNarration,
  resolveAct4TutorialNarration,
  resolveAct4TutorialCompleteNarration,
  type Act4TutorialNarrationCue,
...
import {
  ...
  type Act4StoryNarrationCue,
  type Act4StoryNarrationCueId,
...
import {
  resolveAct4FeedbackNarrationCue,
  type Act4FeedbackCode,
...
const ACT4_STORY_TTS_DEBUG_LABEL = "[ACT4-STORY-TTS]";
const ACT4_PERIOD_TRANSITION_BAR_COUNT = 2;

const isAct4StoryTtsDebugEnabled = () => {
  if (typeof window === "undefined") return false;

  return (
    new URLSearchParams(window.location.search).get("act4StoryTtsDebug") ===
      "1" || window.localStorage.getItem("act4StoryTtsDebug") === "1"
  );
};

type Act4CycleService = {
...
type Act4ControllerOptions = {
  cycle: Act4CycleService;
  ...
  narration?: Act4NarrationService;
};
...
const getSeasonConfig = (
  ...
  const season = resolveAct4SeasonMovementConfig(seasonId, movementValue);

  if (!season) {
    throw new Error(
      `Unknown Act 4 movement "${seasonId}-${movementValue}-percent".`,
    );
  }
...
export const buildAct4TargetsCycleConfig = (
  ...
  seasonDurationMs:
    act4IntroCycleConfig.repetitionCount * act4IntroCycleConfig.barDurationMs,
  repetitionCount: act4IntroCycleConfig.repetitionCount,
  countdownDurationMs: act4IntroCycleConfig.countdownDurationMs,
  seasonalAudioEnabled: true,
});

export const useAct4Controller = ({
  cycle,
  recognition,
  actId,
  debugEnabled,
  ...
  const logStoryTts = (
    ...
    if (!isAct4StoryTtsDebugEnabled()) return;

    console.log(`${ACT4_STORY_TTS_DEBUG_LABEL} ${event}`, details);
  };
  ...
  const warnStoryTts = (
    ...
    console.warn(`${ACT4_STORY_TTS_DEBUG_LABEL} ${event}`, details);
  };
  ...
  const createTutorialNarrationEventId = (
    target: Act4SequenceTarget,
    cueId: string,
  ) => [store.flowId ?? "act4", target.id, cueId].join(":");
  ...
  const playStoryTargetCue = (
    ...
    const resolution = resolveAct4StoryNarrationCue(target);
    ...
    const promise = narration
      .play(resolution.cue.textKey, {
        behavior: "replace",
        params: resolution.params,
        debugLabel: isAct4StoryTtsDebugEnabled()
          ? ACT4_STORY_TTS_DEBUG_LABEL
          : undefined,
  ...
  const playStoryCueById = (
    cueId: Act4StoryNarrationCueId,
    options: Parameters<typeof playStoryCue>[1],
  ) => playStoryCue(act4StoryNarrationCatalog[cueId], options);

  const isTutorialIntroCue = (cue: Act4TutorialNarrationCue) =>
    cue.id.startsWith("act4.tutorial.intro.");

  const isTutorialEncodingCue = (cue: Act4TutorialNarrationCue) =>
  ...
  const startTutorialTargetWithExplanation = async ({
    ...
    const resolution = resolveAct4TutorialNarration({
      target,
      targetIndex,
      flowId: store.flowId,
      includeGlobalIntro,
  ...
  const isReferenceStoryTarget = (
    target: Act4SequenceTarget | null,
  ): target is Act4SequenceTarget & {
    climateData: NonNullable<Act4SequenceTarget["climateData"]>;
  } =>
  ...
  const playClimateStoryIntro = async ({
    ...
    await playStoryCueById("act4.story.intro.chart", {
    ...
    await playStoryCueById("act4.story.intro.reference", {
      target: store.targets[targetIndex] ?? null,
      targetIndex,
      phase: "story-intro",
      status: "storyIntro",
  ...
  const playReferenceCompleteInterlude = async ({
    ...
    await playStoryCueById("act4.story.reference.complete", {
      target: store.targets[targetIndex] ?? null,
      targetIndex,
    ...
    await playStoryCueById("act4.story.reference.scale", {
      target: store.targets[targetIndex] ?? null,
      targetIndex,
      phase: "reference-complete",
      status: "storyReferenceComplete",
  ...
  const playPeriodTransitionNarration = ({
    ...
    const cue = resolveAct4StoryPeriodTransitionCue(completedPeriod);
  ...
  const playClimateCompletionNarration = async (currentRunId: number) => {
    for (const cueId of act4StoryCompletionCueIds) {
      if (!isCurrentRun(currentRunId)) return;
  ...
  const startTargets = async (
    ...
    if (targets[0]?.context === "tutorial") {
      await cycle.prepareCustomCycle(buildAct4TargetsCycleConfig(targets));
    ...
    await cycle.startCustomCycle(buildAct4TargetsCycleConfig(targets), true);
  };
  ...
  const startFullFlow = async () => {
    ...
    await startTargets("act4Full", buildAct4FullSequence(dataset));
  };
  ...
  const startStoryFlow = async () => {
    ...
    await startTargets("act4Story", buildAct4ClimateStorySequence(dataset), {
      includeGlobalIntro: false,
    });
  };

  const startTutorialFlow = async () => {
    await startTargets("act4TutorialDebug", buildAct4TutorialDebugSequence());
  };

  const startDebugSeason = async (season: ClimateSeason) => {
    await startTargets(
      "act4TutorialDebug",
      buildAct4DebugSingleSeasonSequence(season),
      { includeGlobalIntro: false },
    );
  };
  ...
  const startDebugSeasonSequence = async (season: ClimateSeason) => {
    await startTargets(
      "act4TutorialDebug",
      buildAct4DebugSeasonValueSequence(season),
      { includeGlobalIntro: false },
    );
  };

  const pause = () => {
    narrationPausedByAct4 =
      narration.isSpeaking.value && !narration.isPaused.value;
    if (narrationPausedByAct4) {
      narration.pause();
    }
  ...
  const complete = async () => {
    ...
    if (store.flowId === "act4Full" || store.flowId === "act4Story") {
  ...
  const playRetryNarration = (
    ...
    const cueKey = resolveAct4FeedbackNarrationCue(
      target.season,
      feedbackCode as Act4FeedbackCode | null | undefined,
    );
  ...
  const scheduleRetry = (
    ...
    const retryInterludeDurationMs =
      ...
      (act4IntroCycleConfig.barDurationMs / 4);
  ...
  const playSeasonCompletionNarration = async (
    ...
    const cue = resolveAct4SeasonCompletionNarration(target);
  ...
  const schedulePeriodTransition = ({
    ...
    cycle.queueSeasonIndexRestart(
      ...
      {
        interludeDurationMs:
          act4IntroCycleConfig.barDurationMs * ACT4_PERIOD_TRANSITION_BAR_COUNT,
        restartSeasonIndex: nextTargetIndex,
        afterRestart: () =>
  ...
  const getRecognitionPlaybackState =
    (): Act4RecognitionFrame["playbackState"] =>
  ...
  const getRecognitionFrame = (): Act4RecognitionFrame => ({
  ...
  const playDebugOutro = async () => {
    ...
    lastPoseLandmarks = null;
    recognition.resetAll();
    store.startFlow("act4Story", buildAct4ClimateStorySequence(dataset));
    store.completeFlow();
    await cycle.complete();
